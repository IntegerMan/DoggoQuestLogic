...
import {objectResponse} from '../World/GameObject';
...
export class VerbHandler {
  ...
  public handleLook(context: CommandContext): boolean {
    ...
    const target = context.sentence.target;
    if (target) {
      return VerbHandler.handleTargetedVerb('look', target, context, `The ${target.reduced} is pretty much as you'd expect.`);
    } else {
